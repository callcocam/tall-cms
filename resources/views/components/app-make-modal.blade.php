@props(['title' => 'Modal Title', 'btnLabel'=>null])
<div x-data="{ showModal: @entangle('showModal').defer }">
    <button x-on:click="$wire.showModal = true"
        class="btn  p-1 hover:bg-slate-300/20 focus:bg-slate-300/20 active:bg-slate-300/25 dark:hover:bg-navy-300/20 dark:focus:bg-navy-300/20 dark:active:bg-navy-300/25">
        @if($btnLabel)
            <span>{{ $btnLabel }}</span>
        @endif
        <x-tall-icon name="plus" class="ml-2 h-6 w-6 rounded-full"/>
    </button>
    <div class="fixed inset-0 z-[100] flex flex-col items-center justify-center overflow-hidden px-4 py-6 sm:px-5"
        x-show="showModal" role="dialog" @keydown.window.escape="$wire.closeModal()">
        <div class="absolute inset-0 bg-slate-900/60 transition-opacity duration-300" x-on:click="$wire.closeModal()"
        ...
        <div class="relative w-full max-w-2xl origin-bottom rounded-lg bg-white pb-4 transition-all duration-300 dark:bg-navy-700"
            ...
            <div class="is-scrollbar-hidden   overflow-x-auto max-w-6xl block">
                {{ $slot }}
            </div>
